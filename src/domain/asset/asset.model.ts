 ...
 */
export function normalizeFamilyMemberRole(raw: string): FamilyMemberRole {
  return raw === 'parent' || raw === 'child' ? raw : 'parent'
}

/**
 * Normalize asset field
 */
export function normalizeAssetField(raw: string): AssetField {
  switch (raw) {
    case 'fixed_assets':
    case 'current_assets':
    case 'liabilities':
...
export function normalizeAssetCategory(raw: string): AssetCategory {
  const valid: AssetCategory[] = [
    'real_estate', 'retirement_funds', 'cash_savings',
    'investments', 'kids', 'credit_card', 'loans', 'other'
  ]
  return valid.includes(raw as AssetCategory) ? (raw as AssetCategory) : 'other'
}
