...
| `*.service.ts` | Business logic functions | `account.service.ts` |
...
| **amount** | `number` | Stored as **cents** in DB, converted to dollars for display |
...
|------|----------|-------------|
| **Domain** | `src/core/domain/` | Pure types, models, schemas (no I/O) |
| **Services** | `src/core/services/` | Business logic orchestrating domain + infrastructure |
...
| **Shared** | `src/shared/` | Cross-feature reusable code (components, stores, theme) |
...
| **Service** | Business logic functions that orchestrate repositories |
...
|---------|----------|------------|----------|
| `screen` | 22 | 900 (black) | Top of screen titles |
| `section` | 18 | 800 (heavy) | Section titles within body |
| `card` | 16 | 700 (bold) | Card titles |
...
| `tx:*` | `tx:20240102-081200-expense-coffee-4k1s2a` | Transaction key |
| `category.subcategory` | `food.groceries` | Category key (dot notation) |
