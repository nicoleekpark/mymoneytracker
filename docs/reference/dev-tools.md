...
|--------|--------|----------|
| **Seed All** | Inserts fixture data for accounts, transactions, notifications, suggestions, and assets | Populate app with test data for development |
...
| **Seed Drafts** | Inserts draft fixtures only | Quick draft testing without affecting other data |
| **Clear Drafts** | Deletes all drafts | Reset drafts for fresh testing |
| **Reset DB** | Drops all tables, re-migrates, seeds system data AND all fixtures | Full database reset with fresh test data |
...
- **Assets**: Sample asset records

Data source: `src/infrastructure/db/seed/data/*.json`

### Clear All
Deletes fixture data only (accounts, transactions, notifications, suggestions, assets).
Uses fixture keys to identify and remove seeded data.
...
### Seed Drafts / Clear Drafts
For iterative draft testing:
1. **Clear Drafts** - removes all drafts from database
2. **Seed Drafts** - inserts fresh drafts from `seed_drafts.json`
3. Automatically reloads the drafts store after seeding/clearing

**Workflow:** Clear → Seed → Test → Repeat

### Reset DB
**Destructive operation.** Shows confirmation dialog.
Performs full reset with fresh test data:
1. Drops all tables (`resetDbHardDropAllTables`)
2. Re-runs migrations (`migrate`)
3. Seeds system data: categories, accounts, tags (`runSystemSeeds`)
4. Seeds all fixtures: accounts, transactions, notifications, suggestions, assets

Use when you need a completely fresh database with test data.
...
| `src/shared/store/dev.store.ts` | Visibility state (Zustand) |
