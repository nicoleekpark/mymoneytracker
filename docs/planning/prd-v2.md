...
#### 4.2 Hero Context Phrases

**Problem**: Hero comparisons ("Up $2,100 vs last month") lack behavioral context

**AI Solution**:
Analyze spending patterns to add context phrases:

| Pattern Detected | Context Phrase |
|------------------|----------------|
| Low expense variance (< 10%) | "with steady spending" |
| High income spike | "due to bonus/extra income" |
| Seasonal pattern | "typical for this season" |
| Category-driven change | "mainly from [category]" |

**Example**:
```
+$10,726 net
Up $2,100 vs last month with steady spending
```

**Implementation**:
- Calculate expense variance: `stdDev(last3Months) / avg(last3Months)`
- If < 0.1 → "with steady spending"
- If income > 150% of avg → "due to extra income"
- If top category delta > 50% of total delta → "mainly from [category]"
